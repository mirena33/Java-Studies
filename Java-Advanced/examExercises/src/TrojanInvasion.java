    ...
    public static void main(String[] args) {
        ...
        for (int i = 1; i <= waves && !defenders.isEmpty(); i++) {
            ...
            Arrays.stream(scanner.nextLine().split("\\s+")).mapToInt(Integer::parseInt)
                    .forEach(attackers::push); //STACK

            if (i % 3 == 0){
                int additional = Integer.parseInt(scanner.nextLine());
                defenders.offer(additional);
            }
        ...
        if (defenders.isEmpty()){
            ...
            System.out.printf("Warriors left: %s%n", attackers.toString().replaceAll("\\[|\\]+", ""));
        }else {
            System.out.println("The Spartans successfully repulsed the Trojan attack.");
            System.out.printf("Plates left: %s%n", defenders.toString().replaceAll("\\[|\\]+", ""));
        }
